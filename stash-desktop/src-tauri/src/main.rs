...
mod os_impl {
    ...
    pub fn get_open_apps() -> Vec<AppInfo> {
        ...
        // Batch-extract icons for all exe paths
        let paths: Vec<String> = apps.iter().map(|a: &AppInfo| a.exe_path.clone()).collect();
        let icons = extract_icons_batch(&paths);
    ...
    unsafe extern "system" fn enum_windows_callback(hwnd: HWND, lparam: LPARAM) -> BOOL {
        ...
        // Avoid adding duplicates (e.g. multiple windows from same app)
    ...
    pub fn restore_app(app: &AppInfo) {
        let _ = std::process::Command::new(&app.exe_path)
            .creation_flags(0x08000000) // CREATE_NO_WINDOW for console apps
            .spawn();
    }
